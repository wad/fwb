...
public class StatementColor extends Statement {

    private String unevaluatedArguments;

    public StatementColor(String code, BasicRunner basicRunner, int lineNumberInFile) throws BasicException {
    ...
    public void execute() throws BasicException {
        if (unevaluatedArguments.length() == 0) {
            ...
            int r = (int) new ValueNumber(arguments[0]).toLong();
            int g = (int) new ValueNumber(arguments[1]).toLong();
            int b = (int) new ValueNumber(arguments[2]).toLong();

            getBasicRunner().getGraphicsTerminal().color(r, g, b);
        }
    }
