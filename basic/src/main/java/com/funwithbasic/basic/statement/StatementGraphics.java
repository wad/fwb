...
public class StatementGraphics extends Statement {

    private String unevaluatedArguments;

    public StatementGraphics(String code, BasicRunner basicRunner, int lineNumberInFile) throws BasicException {
    ...
    public void execute() throws BasicException {
        if (unevaluatedArguments.length() == 0) {
            getBasicRunner().getGraphicsTerminal().create();
        }
        else {
            ...
            int s = (int) new ValueNumber(arguments[0]).toLong();
            int x = (int) new ValueNumber(arguments[1]).toLong();
            int y = (int) new ValueNumber(arguments[2]).toLong();

            if (s <= 0 || x <= 0 || y <= 0) {
                invalid(me(), unevaluatedArguments, "invalid graphics parameters specified");
            }
            getBasicRunner().getGraphicsTerminal().create(s, x, y);
        }
    }
